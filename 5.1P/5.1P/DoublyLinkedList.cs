...
namespace DoublyLinkedList
{
    public class DoublyLinkedList<T>
    {
        ...
        {
            Node<T> new_node = before as Node<T>;
            return AddBetween(value, new_node.Previous, new_node);
        }

        public INode<T> AddAfter(INode<T> after, T value)
        {
            Node<T> new_node = after as Node<T>;
            return AddBetween(value, new_node, new_node.Next);
        }
        ...
        {
            Node<T> new_node = node as Node<T>;
            INode<T> node1 = Find(new_node.Value);
            Node<T> pre = new_node.Previous;
            Node<T> Next = new_node.Next;
            if(node1 == null)
            {
